...
if __name__ == "__main__":
	...
	print(HACS_VID_DIR)
	#
	meta_path = '/braintree/home/fksato/HACS_total/total/global_meta.pkl'
	#
	...
	for action in actions_list:
		...
		recorded_df, dist_list = check_dloaded(anno_list, copy_dir, dloaded_action_meta, move_files=PROD
		                                       , retrieve_meta=True)
		previously_dloaded.append(recorded_df)
		tot_vid_cnt += len(dist_list)
		dist_list = round_robin(dist_list, MAX_CPU)
	...
	previously_dloaded.to_pickle(os.path.join(HACS_VID_DIR, 'segment/previously_dloaded.pk'))

	print(f'Total video count for download: {tot_vid_cnt}')

	pool = mp.Pool(MAX_CPU)
	offsets = [0] + [len(x) for x in complete_dist[:-2]]
	dloaders = []
	for proc in range(MAX_CPU):
